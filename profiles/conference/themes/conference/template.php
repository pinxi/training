 ...
 */
function conference_preprocess_page(&$vars) {
  ...
  if (arg(0) != 'admin' && !empty($vars['right']) && !$layout) {
    ...
    $css = array('screen' => array('theme' => array(drupal_get_path('theme', 'conference') . '/layout-sidebar.css' => TRUE)));
 ...
 */
function conference_preprocess_node(&$vars) {
  $vars['layout'] = FALSE;
  $vars['submitted'] = '';
}
 ...
 */
function conference_preprocess_comment(&$vars) {
  $vars['layout'] = FALSE;
}
